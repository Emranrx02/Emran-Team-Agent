    ...
    <div className={styles.sidebar}>
      <div className={styles.sidebarHeader}>
        <div className={styles.logo}>
          ...
          <div>
            <div className={styles.logoText}>Agent<span>Forge</span></div>
            <div className={styles.powered}>Powered by GPT-4o</div>
          </div>
        </div>
...
function SetupPanel({ project, files, onSave, onFileAdd, onFileRemove }) {
  ...
  const train = async () => {
    ...
    try {
      ...
      const reply = await callOpenAI(sys, [{ role: 'user', content: 'Confirm you are ready. Reply in one short sentence in the project tone.' }])
...
function ChatPanel({ project }) {
  ...
  const send = async () => {
    ...
    try {
      const reply = await callOpenAI(project.systemPrompt, newMsgs.map(m => ({ role: m.role, content: m.content })))
